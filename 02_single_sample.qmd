...
p_influential <- meta_one_sample_influential$Data |> 
  ...
  geom_text_repel(aes(label = Author), max.overlaps = 4) +
  geom_point(alpha = .5)  +
...
ggsave("figures/paper/p_one_sample_cpz.pdf", p_one_sample_cpz, 
       width = 5, height = 5)

ggsave("figures/paper/p_one_sample_cpz.png", p_one_sample_cpz, 
       width = 5, height = 5)
```
...
ggplot(studies_one_sample_chronic |> 
                             drop_na(size), 
                           aes(x = size, y = es)) +
    geom_abline(intercept = meta_one_sample_chronic_size |> 
                tidy() |> 
                filter(term == "intercept") |> 
                pull(estimate), 
              slope = meta_one_sample_chronic_size |> 
                tidy() |> 
                filter(term == "size") |> 
                pull(estimate), 
              color = "grey") +
  geom_point() +
  geom_text_repel(aes (label = study), size = 2.5) +
  labs(x = "Size (deg)", y = LABEL_EFFECT_SIZE) +
  theme_classic(16) +
    theme(axis.line = element_line(linewidth = WIDTH_AXIS),
        axis.ticks = element_line(linewidth = WIDTH_AXIS))
...
ggplot(studies_one_sample_chronic |> 
                             drop_na(p_gender), 
                           aes(x = p_gender, y = es)) +
    geom_abline(intercept = meta_one_sample_chronic_p_gender |> 
                tidy() |> 
                filter(term == "intercept") |> 
                pull(estimate), 
              slope = meta_one_sample_chronic_p_gender |> 
                tidy() |> 
                filter(term == "p_gender") |> 
                pull(estimate), 
              color = "grey") +
  geom_point() +
  geom_text_repel(aes (label = study), size = 2.5) +
  labs(x = "Proportion females", y = LABEL_EFFECT_SIZE) +
  theme_classic(16) +
    theme(axis.line = element_line(linewidth = WIDTH_AXIS),
        axis.ticks = element_line(linewidth = WIDTH_AXIS))
```
...
```{r}
p_influential_funnel <- plot_grid(p_influential, 
                                  p_one_sample_funnel, 
                                  NA,
                                  ncol = 1, 
                                  rel_heights = c(.4, .4, .2),
...
p_influential_funnel_forest <- plot_grid(p_influential_funnel, 
